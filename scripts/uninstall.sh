#!/usr/bin/env bash
# Uninstall Claude Switcher: the app bundle, its Stop/StopFailure hook, its files under
# ~/.claude/accounts/.switcher, its preferences and login item. The claude-account store
# (snapshots, profiles, .quota, live login) is left untouched.
#
# From a downloaded .dmg run it as:  bash "/Volumes/Claude Switcher 0.1.0/Uninstall.command"
...
if [ -x "$APP/Contents/MacOS/ClaudeSwitcher" ]; then
  ...
  rm -rf "$dir/.switcher" "$HOME/.local/bin/claude-switcher-hook"
  s="$HOME/.claude/settings.json"
  if [ -f "$s" ] && command -v jq >/dev/null 2>&1 && grep -q claude-switcher-hook "$s"; then
    cp "$s" "$s.bak-$(date +%s)"
    # keep in sync with HookInstaller.unwireSettings
    jq 'if .hooks then
          .hooks |= with_entries(.value |= (if type == "array" then map(select(any(.hooks[]?; (.command // "") | contains("claude-switcher-hook")) | not)) else . end))
        else . end' "$s" > "$s.tmp" && mv "$s.tmp" "$s"
  fi
...
echo "Claude Switcher uninstalled. claude-account snapshots, .quota files and the live login were not touched."
